...
class Waypoint:
    def __init__(self, ship, ns=0, ew=0):
        self._dirs = ['N', 'E', 'S', 'W']
        self._ns = ns
        self._ew = ew
        self._ship = ship

    def move_waypoint(self, dirr, steps):
        if dirr == 'N':
            self._ns += steps
        elif dirr == 'S':
            self._ns -= steps
    ...
    def rotate_waypoint_right(self, angle):
        transformations = [
            lambda x, y: (y, x),   # WN -> NE
            lambda x, y: (-y, x),  # NE -> ES
            lambda x, y: (-y, x),  # ES -> SW
            lambda x, y: (-y, -x)  # SW -> WN
        ]

        r = angle % 360 // 90
        assert(isinstance(r, int))
        x = self._dirs.index('N' if self._ns > 0 else 'S')
        y = self._dirs.index('E' if self._ew > 0 else 'W')
        x_0 = (x + r) % len(self._dirs)
        y_0 = (y + r) % len(self._dirs)
        for i in range(y, y_0):
            self._ns, self._ew = transformations[i](self._ns, self._ew)
...
class Ship:
    def __init__(self, dirr):
        ...
        self._ns = 0
        self._ew = 0
        self._waypoint = Waypoint(self, 1, 10)
